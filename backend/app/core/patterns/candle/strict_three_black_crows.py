"""Strict three black crows — three bearish bars with strict body/wick ratios.

Symmetric counterpart to :mod:`strict_three_white_soldiers`:
- each body ≥ 0.7 × ATR(14)
- max wick ≤ 25 % of that bar's body
- each close strictly lower than the previous
"""
...
class StrictThreeBlackCrowsPattern:
    pattern_id: str = "strict_three_black_crows"
    ...
    def detect(
        ...
        if not all(is_bearish(b) for _, b in last3.iterrows()):
            return None
