"""Saucer bottom — long, shallow concave-up bottom (gentler than rounding bottom)."""
from __future__ import annotations

import numpy as np
import pandas as pd

from app.core.patterns.base import PatternFire, PatternType


class SaucerBottomPattern:
    pattern_id: str = "saucer_bottom"
    pattern_type: PatternType = "chart"
    LOOKBACK = 80

    def detect(
        self, bars: pd.DataFrame, current_idx: int
    ) -> PatternFire | None:
        if current_idx < self.LOOKBACK:
            return None
        win = bars.iloc[current_idx - self.LOOKBACK : current_idx + 1]
        lows = win["low"].to_numpy(dtype=float)
        xs = np.arange(len(lows), dtype=float)
        a, b, c = np.polyfit(xs, lows, 2)
        apex_x = -b / (2 * a) if a != 0 else -1
        if a <= 1e-6 or apex_x < 0 or apex_x > len(lows):
            return None
        price_range = float(lows.max() - lows.min())
        # Saucer is shallow but visible: depth vs mean must be in [0.5%, 30%]
        depth_ratio = price_range / float(lows.mean())
        if depth_ratio < 0.005 or depth_ratio > 0.3:
            return None
        fit = a * xs**2 + b * xs + c
        ss_res = float(np.sum((lows - fit) ** 2))
        ss_tot = float(np.sum((lows - lows.mean()) ** 2)) + 1e-9
        r2 = 1.0 - ss_res / ss_tot
        if r2 < 0.6:
            return None
        return PatternFire(
            pattern_id=self.pattern_id,
            direction="LONG",
            strength=0.55,
            confidence=0.5,
            evidence={
                "curvature_a": float(a),
                "r2": r2,
                "apex_x": float(apex_x),
            },
        )
