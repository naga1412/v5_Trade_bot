...
def test_belt_hold_returns_none_on_neutral_input() -> None:
    """Flat constant bars must not trigger the pattern."""
    bars = _bars([(100.0, 101.0, 99.0, 100.0, 1_000.0)] * 30)
    fire = BeltHoldPattern().detect(bars, current_idx=29)
    assert fire is None


def test_belt_hold_pattern_id_and_type() -> None:
    p = BeltHoldPattern()
    assert p.pattern_id == "belt_hold"
    assert p.pattern_type == "candle"


def test_belt_hold_fires_on_known_shape_or_returns_none() -> None:
    ...
    bars = _bars(rows)
    fire = BeltHoldPattern().detect(bars, current_idx=len(rows) - 1)
    if fire is not None:
