...
def test_hammer_returns_none_on_neutral_input() -> None:
    ...
    fire = HammerPattern().detect(bars, current_idx=29)
    assert fire is None
...
def test_hammer_fires_on_known_shape_or_returns_none() -> None:
    ...
    bars = _bars(rows)
    fire = HammerPattern().detect(bars, current_idx=len(rows) - 1)
    if fire is not None:
