...
async def test_open_position_survives_simulated_fleet_supervisor_restart(session_factory) -> None:
    ...
    # Promotion note (Stage 1, 2026-08-31): the upstream assertion also
    # checked a 3rd tuple element (cohort tag) -- that's cohort-threading
    # (Phase 4 Task 9, Epic B), not part of Stage 1. _load_keepalive_symbols
    # returns a plain (symbol_pair, timeframe) 2-tuple here; the real thing
    # this test proves -- the rescued position survives into a completely
    # independent fresh keepalive load -- is unaffected by dropping the
    # cohort assertion.
