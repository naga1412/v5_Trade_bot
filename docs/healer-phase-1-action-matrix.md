...
**Amended 2026-07-28 (later same day, post prod-promotion #29):** Row 7 is
**RESTORED to CONDITIONALLY APPROVED**, unblocking the earlier BLOCKED PENDING
ROOT CAUSE status. Root cause was identified as `KEEPALIVE_TOP_N=20` cap
against a 30-symbol universe — ranks 21-30 are structurally outside live
coverage BY DESIGN (data-backed ratification in `docs/superpowers/decisions/
2026-07-28-fleet-cap-top-20-ratified.md`). C3 was narrowed to fleet-eligible
top-20 + singleton via PR #367→#368 (main `8a245df`); verified 2026-07-28 with
C3 stale-findings dropping from 9/27/tick to 0/tick. Under the new scope, a
C3 warning now means "a symbol that SHOULD have a child went stale" — the
exact condition a WS-child restart can fix. The original BLOCKED objection
(restarting a child that was never spawned) no longer applies. Observation
window RESET to start from the #368 deploy: **2026-07-28 11:55 UTC → target
2026-08-04 11:55 UTC**. The prior window accrued against a detector firing
~10 structural false-ish positives per tick and does not count. Same 3-
condition test unchanged: clean window + selftest confirmed (already
satisfied) + explicit operator GO. B1-dependent row unaffected (matures
2026-07-31 05:23 UTC as before).
...
| **Recommendation** | ✅ **CONDITIONALLY APPROVED (restored 2026-07-28 post prod-promotion #29)** — the BLOCKED PENDING ROOT CAUSE downgrade earlier the same day is resolved. Root cause identified as `KEEPALIVE_TOP_N=20` cap vs 30-symbol universe (see `docs/superpowers/decisions/2026-07-28-fleet-cap-top-20-ratified.md`). C3 was narrowed to fleet-eligible top-20 ∪ singleton via PR #367→#368 (main `8a245df`); verified 2026-07-28 with stale-findings dropping from 9/27/tick to 0/tick. Under the narrowed scope, a C3 warning now means "a symbol that SHOULD have a child went stale" — the exact condition a WS-child restart can fix; the original BLOCKED objection (nothing to restart) no longer applies.<br><br>**Observation window RESET to start from #368 deploy: 2026-07-28 11:55 UTC → target 2026-08-04 11:55 UTC.** The prior window accrued against a detector firing ~10 structural false-positives per tick and does not count. Same 3-condition test unchanged: clean window + selftest confirmed (already satisfied) + explicit operator GO. B1-dependent Row 10 unaffected (still matures 2026-07-31 05:23 UTC).<br><br>**Historical amendments preserved:**<br>• **2026-07-23**: CONDITIONALLY APPROVED for Phase 1 — cleanest first Phase-1 auto-action; low blast, high leverage. Condition: 7-day Phase-0 clean run + selftest confirmation + explicit operator GO.<br>• **2026-07-24 (post prod-promotion #27)**: PREREQUISITE ADDED — the C3 redesign (boot grace + severity split shipped as Phase 0 completion) MUST land AND run its own clean observation window BEFORE this row's auto-restart merges. Auto-restarting on the previous C3 shape = churn every deploy (85 warnings observed on the #27 first tick). The redesign fixes that; the auto-restart layer builds on the fixed detector, not the noisy one.<br>• **2026-07-28 (post prod-promotion #28)**: DOWNGRADED to BLOCKED PENDING ROOT CAUSE — post-#362 investigation of C3-flagged stale majors (LINK 50d, UNI 13d, LTC 6d, plus rolling drops on XLM/PUMP/ADA/ONDO/ENA/SUI/NEAR/U) showed prod's backend was recreated ≥5 times in the preceding 6 days with the stale symbols surviving every restart. Container recreate rebuilds every WS subscription from scratch; a per-symbol WS-child restart could not heal what a full-stack restart already ignored. Auto-restart would loop-fire without changing the outcome. Reinstatement gated on root-cause identification + demonstration that WS-child restart resolves the class.<br>• **2026-07-28 (later same day, post prod-promotion #29)**: RESTORED — root cause identified (fleet cap vs universe size), C3 scope narrowed, warning semantics now match Row 7's assumed condition. See top of this doc's header amendment paragraph for the full closeout narrative. |
