...
- **New (2026-08-20): alembic migration `0040_shadow_open_positions_remaining_cols` is a hard dependency of this plan and must reach `main` before Task 17 (turning `futures_poll_task` on) or before reversal criterion #2's cohort re-measurement is trusted, whichever comes first.** Pre-0040, `shadow_open_positions` doesn't carry `symbol_source` (added by Task 3's migration 0038 to `shadow_trades` only) — a shadow position that survives a worker restart between open and close relabels to the `"established_top20"` dataclass default on close, regardless of its real cohort. This is a silent MISCLASSIFICATION, not a null: a null-filtered analysis would not catch it. Criterion #2 is a `shadow_trades`-based three-way cohort split (see Task 3's redraft note and the 2026-08-15 decision record); any restart-survivor row closed before 0040 lands corrupts both the `established_top20` baseline (contaminated with misfiled new-cohort trades) and the new-cohort counts (undercounted) for that split. See `docs/superpowers/decisions/2026-08-15-liquidity-floor-selector-supersedes-top20.md` for the criterion #2 text and the staging-data flag this requires (Task 18 Step 3 addendum).
...
## Task 5f: Fleet reconciliation cadence — root cause + correct value (new, ratified 2026-08-19)

> **New task, found while confirming Task 5e's fix on staging.** `ws_keepalive_task` and `futures_poll_task` both picked up the freshly-populated `live_fleet_universe` table within ~5 minutes of a fresh deploy — but investigation showed this was accidental, not designed, and the underlying reconciliation-cadence constants are stale from before the universe-refresh cadence moved to 6h (Task 5c). Two separate problems, both fixed here:
>
> **Problem 1 — `KEEPALIVE_REFRESH_SECONDS`/`FUTURES_POLL_REFRESH_SECONDS` are still `24 * 60 * 60`, four times the new 6h universe-refresh cadence.** A newly-qualifying symbol can sit unpolled for up to a full day after the selector picks it up; a dropped one lingers just as long. This directly defeats the reason the cadence moved to 6h at all, and — decisively — it invalidates Task 18's item 10 (hysteresis flapping cannot be observed if the fleet only reconciles once across the whole 24h soak window).
>
> **Problem 2 (the actual root cause of why Problem 1 didn't visibly manifest on the last deploy) — `last_refresh = 0.0` in both `run_keepalive` and `run_futures_poll`'s main loops.** The comparison `now - last_refresh >= refresh_seconds` uses `now = loop.time()`, which is backed by `time.monotonic()` → the kernel's `CLOCK_MONOTONIC`. Docker containers share their host's kernel (no per-container clock namespace), so this clock reflects host-wide elapsed time (typically since host boot), not "seconds since this process/container started." On the Hetzner staging host — up far longer than any `refresh_seconds` value — `now` was already large enough that `now - 0.0 >= refresh_seconds` was trivially true on the very first in-loop check, so reconciliation fired almost immediately after startup by accident, not because a genuine `refresh_seconds`-length wait elapsed. **A host reboot occurring near a container restart would reset this clock and cause the identical code to genuinely wait the full nominal interval before ever reconciling — an environment-dependent divergence between a long-running host and a freshly-rebooted one, the worst possible place for behavior to differ.** Fix: initialize `last_refresh = loop.time()` (captured right after the initial population step, not a hardcoded `0.0`), so the gate correctly measures real elapsed time since the *last reconciliation*, independent of host uptime or clock reference point.
>
> **Cadence value: 1h, not 6h.** Reconciliation itself (`_load_keepalive_symbols`/`_load_desired_futures_symbols` + diff-and-spawn/cancel) is cheap — a `SELECT` against the already-refreshed `live_fleet_universe` table plus a dict diff, no fresh `check_liquidity` sampling (that only happens in the 6h universe refresh itself). Checking hourly caps the fleet's lag-behind-the-universe-table at one candle interval instead of up to a full 6h, for a negligible cost (23 extra background checks/day, each a single indexed `SELECT`).

**Files:**
- Modify: `backend/app/ws/keepalive.py` (`KEEPALIVE_REFRESH_SECONDS`, `run_keepalive`'s `last_refresh` initialization)
- Modify: `backend/app/ws/futures_poll.py` (`FUTURES_POLL_REFRESH_SECONDS`, `run_futures_poll`'s `last_refresh` initialization)
- Modify: `backend/tests/unit/test_ws_keepalive.py`, `backend/tests/ops/test_futures_poll_task.py` (or wherever the real current test files live — verify paths, this rollout has repeatedly found the plan doc's assumed paths drift)

**Interfaces:** unchanged — `run_keepalive`/`run_futures_poll`'s signatures don't change, only the default `refresh_seconds` value and the internal `last_refresh` initialization.

- [ ] **Step 1: Fix the constants**

```python
# backend/app/ws/keepalive.py
KEEPALIVE_REFRESH_SECONDS: int = 60 * 60  # 1h (was 24h -- stale since Task 5c moved the universe refresh to 6h)
```
```python
# backend/app/ws/futures_poll.py
FUTURES_POLL_REFRESH_SECONDS: int = 60 * 60  # 1h (was 24h -- see keepalive.py's identical fix)
```

- [ ] **Step 2: Fix the root cause in both `run_keepalive` and `run_futures_poll`**

Change `last_refresh = 0.0` to `last_refresh = loop.time()`, placed AFTER `loop = asyncio.get_event_loop()` is assigned (need the loop reference before calling `.time()` on it) and AFTER the initial-population heartbeat, so it captures "now" at the moment the initial population completed — not before it (an initial population that itself takes a while, e.g. if `_load_keepalive_symbols` is slow, should not count against the first hour). Add a code comment explaining why this isn't a hardcoded `0.0` — cite the shared-kernel-clock reasoning above, since a future reader might otherwise "simplify" it back.

- [ ] **Step 3: Add a regression test for the root cause specifically**

Read the real current test files first (`backend/tests/unit/test_ws_keepalive.py` and wherever `futures_poll`'s tests live — confirm exact paths, don't assume). Add a test that mocks `asyncio.get_event_loop().time()` (or injects a fake `loop.time` via whatever this repo's existing test convention is for `run_keepalive`/`run_futures_poll` — check `_run_child_with_restart`'s or the loop's own existing tests for the pattern) to return a LARGE value (simulating a long-uptime host, e.g. `100_000.0` seconds) starting from the very first call, and assert reconciliation does NOT fire until a genuine `refresh_seconds` has elapsed in SIMULATED loop-time terms after the initial population — not immediately just because the absolute `loop.time()` value happens to exceed `refresh_seconds`. This is the test that would have caught the actual bug (the old `last_refresh = 0.0` code would have reconciled immediately under this exact scenario, which is precisely what happened on staging).

- [ ] **Step 4: Run the regression suite**

```
cd backend && python -m pytest --no-cov -p no:cacheprovider tests/unit/test_ws_keepalive.py tests/ops/test_futures_poll_task.py tests/unit/test_futures_poll_candles.py tests/unit/test_futures_poll_watermark.py -v
```
(Adjust paths to whatever Step 3 confirms is real — the plan doc's assumed paths for futures_poll tests have drifted before, e.g. Task 5d's implementation found the same issue.)

- [ ] **Step 5: Commit**

```bash
git add backend/app/ws/keepalive.py backend/app/ws/futures_poll.py backend/tests/unit/test_ws_keepalive.py
git commit -m "fix(phase4): fleet reconciliation cadence 24h->1h + fix loop.time() root cause behind why it worked on staging by accident"
```
...
- [ ] **Step 3b (added 2026-08-20): flag any staging row closed before migration 0040 landed, before trusting a cohort split.** Phase 4 has been running on staging since before 0040 existed, so restart-survivor `shadow_trades` rows closed in that window may carry a misclassified `symbol_source` (silently relabeled to `"established_top20"`, not null — see the Global Constraints note above). Run: `SELECT symbol_source, COUNT(*), MIN(closed_at), MAX(closed_at) FROM shadow_trades WHERE closed_at < '<0040 deploy timestamp>' AND symbol_source = 'established_top20' GROUP BY symbol_source` and cross-reference against `signal_id` overlap with any `liquidity_added_spot`/`futures_poll` row in `predictions` for the same `(symbol, ts)` — a match indicates a mislabeled restart-survivor. Exclude flagged rows from any cohort-split analysis feeding reversal criterion #2, or re-derive their true cohort from the `predictions` row before including them.
