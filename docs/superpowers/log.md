...
## 2026-05-03 — SP-5 Full Scoring + Traps: SHIPPED

**Scope:** completed the 10-layer scoring stack that fuses macro, patterns,
momentum, SMC, volume, micro patterns, ML hookups, news, and a brain
multiplier into a single FINAL_SCORE; layered on a 17-detector trap stack
(12 main + 5 short-only) with Trap Protocol + `TrapFire` + `TrapContext`
plumbing; extended the aggregator with the full MASTER_PLAN §5 formula
(`static * brain * trap_factor * news * direction_penalty`); added asymmetric
SHORT-bias tier classification (NO_SIGNAL / PAPER / SMALL / STANDARD / A+
with +10pp threshold shift on shorts); built a 50-fixture cross-validation
harness so any future aggregator drift is detected by replay; wired
`build_prediction()` end-to-end (every closed candle now scores all 10
layers, runs all 17 traps, derives the tier, and persists the enriched
payload to `predictions.layer_scores` JSONB); shipped two admin REST
endpoints for trap enable/disable mirroring SP-2's pattern admin contract.

**Delivered (~37 commits on branch `sp-5/main`):**

| Phase | Sub-system                                                                                          | Commits |
|-------|-----------------------------------------------------------------------------------------------------|---------|
| A     | Worktree + migration 0011 (`trap_enabled`) + spec/plan                                              | 4       |
| B     | New layers — L4 SMC + L6 micro + L7/L8/L9/L10 placeholders/hookups                                  | 6       |
| C     | 12 main trap modules (`pre_news_event`, `liquidity_sweep`, `parabolic_blowoff`, `friday_weekend`, `counter_weekly`, `all_indicator_extreme`, `alt_btc_indecision`, `volume_no_followthrough`, `pattern_in_pattern`, `thin_orderbook`, `price_extreme`, `volatility_regime_change`) + base Protocol | 12 |
| D     | 5 short-only traps (`short_squeeze_cascade`, `funding_rate_decay`, `borrow_rate`, `unlimited_upside_risk`, `regulatory_short_ban`) | 5 |
| E     | Aggregator extension + tier classifier + cross-check harness + trap orchestrator + full-pipeline E2E | 5 |
...
- Pre-existing Windows-only `test_ml_checkpoints` failure (`Path('.')`
  ...
  remain pre-existing and unrelated.

**Surprises / decisions worth flagging:**

- **L4 SMC fallback for monotonic bars.** The classic SMC swing-pivot
  detection (`scipy.signal.find_peaks`) returns `[]` on a clean
  monotonic uptrend (e.g. `np.linspace(100, 200, 250)`), so the BoS vote
  would never fire on synthetic test data. We fall back to a 20-bar
  rolling-max/min range break when no swing pivots are found — preserves
  the spec semantics on real data while letting the layer produce a
  meaningful direction in degenerate test cases.
- **L6 micro patterns reuse SP-2's hand-rolled subset.** The plan listed
  conceptual ids (`marubozu_bull`, `engulfing_bull`) that don't exist as
  separate registry entries — SP-2's candle patterns return signed
  direction inside `PatternFire`. The curated micro set draws from real
  registered ids (pinbar_{long,short}, inside_bar_breakout_{long,short},
  key_reversal_{high,low,long,short}, rejection_wick_at_{resistance,
  support}, etc.) with the registry membership pinned by
  `test_micro_pattern_ids_resolve_to_registered_patterns`.
- **Trap penalty capped at 4 fires.** Five fires equals four fires equals
  `(1 - 0.15)^4 = 0.522` — beyond that the multiplier saturates.
  Intentional per MASTER_PLAN §5: a parabolic blowoff that triggers seven
  separate detectors should not produce an effectively-zero score that
  the operator can't reason about. Worst-case multiplier is 0.522.
- **Cross-check uses fixture-generator-encoded formula.** The 50 fixtures
  are generated by a pure-Python encoding of the FINAL_SCORE formula;
  the cross-check then runs the same inputs through the live
  `aggregator.aggregate(...)` and diffs the results within 1e-3. Any
  future formula drift between the aggregator and the encoded reference
  produces a loud test failure rather than a silent semantic change.
- **Frontend admin sub-page deferred to SP-6.** F2's
  `GET/POST /api/v1/admin/traps[/{trap_id}/{disable,enable}]` endpoints
  ship live and are exercised by the new `test_api_admin_traps.py`
  integration suite (10 cases). The React UI to drive them is pushed to
  SP-6 alongside the SP-2 `/patterns` admin page and SP-3 `/adapters`
  admin page so we can land all three admin sub-pages in one frontend
  push instead of bouncing the design system three times.
- **`prediction_extras` is a separate JSONB carrier.** Rather than widen
  the typed `LivePredictionOut.layer_scores: dict[str, LayerScoreOut |
  None]` to a `dict[str, Any]` (which would break the API contract and
  the frontend type generator), the predictor exposes a new
  `prediction_extras: dict[str, Any] | None` field that the persistence
  sites (live worker + shadow worker) merge into the JSONB column on the
  way to disk. The frontend reads only `layer_scores`; the audit/replay
  path picks up `extras` from the persisted JSONB.
- **TrapContext live data sources still TODO.** `funding_rate`,
  `open_interest_delta_24h`, `borrow_rate_pct`, and
  `next_news_event_minutes_until` are all `None` in F1's wiring. The
  cheap bar-derived fields (`is_friday_close`, `weekly_bias`,
  `btc_atr_pct`) compute locally so the seven traps that need them stop
  being blind. The four blind ones gracefully no-op (return `None`) per
  the Trap Protocol contract — a missing context field is not a fire.
  Live data wiring lands in SP-3.5 (funding/OI/borrow adapters) and
  SP-9 (news event ingest).
- **Pre-existing Windows test_ml_checkpoints failure unrelated.** Same
  `Path('.')` permission issue surfaced before SP-2 and intentionally
  not papered over here.

**Manual checklist (for the human after deploy):**
- [ ] Migration 0011 (`trap_enabled`) applies cleanly on the staging Postgres
- [ ] `curl .../api/v1/admin/traps` returns 17 entries with default `enabled: true`
- [ ] After one closed candle in production, a fresh row in `predictions`
      shows `layer_scores->>'tier'` set and `layer_scores->>'traps_fired'`
      as a JSON array (possibly empty)
- [ ] `curl -X POST .../api/v1/admin/traps/parabolic_blowoff/disable -d '{}'`
      returns 200, then a follow-up GET shows `enabled: false`

**Next:** ship F5 (PR `sp-5/main` → `main`) and tag `sp-5`. Then either
- **SP-1.5** — train the L7 XGBoost on the 43-indicator feature vector
  and wire inference, or
- **SP-9** — FinBERT + news API ingest to populate L9 + the
  `next_news_event_minutes_until` and `news_multiplier` channels, or
- **SP-4** — train the L10 RL meta-brain (PPO) and wire BRAIN_ADJUST live
  signal into the aggregator, or
- **SP-3.5** — funding/OI/borrow adapter additions to fill the four blind
  fields on `TrapContext`, or
- **SP-6** — frontend admin sub-pages for patterns / traps / adapters /
