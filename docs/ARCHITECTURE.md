...
| 13 | Audit hash chain | `app/db/audit.py` | Append-only prev_hash → row_hash on 4 tables |
...
Append-only hash chain on `predictions`, `paper_trades`,
`shadow_trades`, `live_trades`. Each row stores `prev_hash` + `row_hash`
so any tampering is detectable. Nightly `audit_verifier_task` walks each
table and alerts on any break.
